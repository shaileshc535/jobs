 <?php
      ...
      $sql = "SELECT * FROM job_seeker_details LIMIT {$offset},{$limit_per_page}";
     ...
     <h2><a href='candidate-detail.php?id={$row["user_id"]}'>{$row["profession"]}</a> <i class='careerfy-icon careerfy-check-mark'></i></h2>
         <ul>
             <li>{$row["fname"]} {$row["lname"]}<a href='candidate-detail.php?id={$row["user_id"]}' class='careerfy-candidate-default-studio'></a></li>
     ...
     <a href='candidate-detail.php?id={$row['user_id']}' class='careerfy-candidate-default-btn'><i class='careerfy-icon careerfy-add-list'></i>View</a>
 </div>
     </div>
    </li>
    ";
